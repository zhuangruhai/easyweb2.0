...
public class RoleServiceImpl implements RoleService {
	...
	public void createRole(Role role) throws Exception {
		...
		// insertRoleResourceOperation(role.getRoleId(),
		// roleResourceOperations);
	...
	private void addResourceCategory(TreeBuilder tree) {
		...
		for (ResourceCategory ca : rcs) {
			TreeNode node = new TreeNode(longToString(ca.getCategoryId()), longToString(ca.getParentId()),
					ca.getCategoryName());
			tree.addNode(node);
		}
	}
	...
	private void addResource(TreeBuilder tree) {
		...
		for (com.hichlink.easyweb.portal.common.entity.Resource res : rList) {
			TreeNode node = new TreeNode("res-" + longToString(res.getResourceId()), longToString(res.getCategoryId()),
					res.getResourceName());
			tree.addNode(node);
		}
	}
	...
	private void addOperation(TreeBuilder tree) {
		...
		for (Operation op : oList) {
			...
			CheckTreeNode node = new CheckTreeNode(op.getResourceId() + "-" + op.getOperationKey(),
					"res-" + op.getResourceId(), op.getOperationName());
			tree.addNode(node);
		}
