...
public class StaffServiceImpl implements StaffService {
	...
	public void updateStaff(Staff staff) throws Exception {
		...
		if (StringTools.isNotEmptyString(staff.getMobile()) && !staff.getMobile().equalsIgnoreCase(oldStaff.getMobile())
				&& isStaffMobileExsit(staff.getMobile())) {
			throw new Exception("手机号码已被注册，系统不允许重复注册!");
		}
		if (StringTools.isNotEmptyString(staff.getEmail()) && !staff.getEmail().equalsIgnoreCase(oldStaff.getEmail())
				&& isStaffEmailExsit(staff.getEmail())) {
			throw new Exception("邮箱地址已被注册，系统不允许重复注册!");
		}
	...
	private String buildStaffPassword(Staff staff, String password) {
		...
		// return PasswordUtil.buildPasswordWithoutVerify(staff.getLoginName(),
		// password);
	...
	/*
	 * public void updateStaffDepartment(String departmentId, String staffids)
	 * throws Exception{ Map<String,String> map = new HashMap<String,String>();
	 * 
	 * map.put("departmentId", departmentId); map.put("staffIds", staffids);
	 * 
	...
	//
	//// int passwordExpirePeriod =
	// SecurityConfig.getConfig().getInt("password-history.password-expire-period",
	// 30);
	// int passwordExpirePeriod =
	// SecurityConfig.getSecurityConfig().getPasswordExpirePeriod();
	//
	// // Date passwordExpireDate = DateUtils.addDays(nowDate,
